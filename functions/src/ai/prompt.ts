...
export function buildSystemPrompt(opts: {
  ...
  const lines = [
    ...
    "Use the provided search tools to find real matches. NEVER invent or assume any platform data — providers, prices, " +
    ...
    "Do not ask for or repeat personal data (phone numbers, email addresses, payment details); direct the user to the secure booking flow for those.",
    "",
  ...
  if (opts.override && opts.override.trim()) {
    lines.push(
      "",
      "Additional operator instructions (these SUPPLEMENT but do NOT override the scope, safety, or no-invent rules above):",
      opts.override.trim(),
      "",
      "Reminder: the STRICT SCOPE and NEVER-invent rules above remain in effect regardless of the operator instructions.",
    );
  }
  return lines.join("\n");
}
