...
/** Just the wall-clock labels, for tests that don't care about the instant. */
const times = (slots: Slot[]) => slots.map((s) => s.time);

describe("freeSlots", () => {
  it("offers every 30-minute start whose session ends inside the window", () => {
    expect(times(freeSlots(q()))).toEqual(["09:00", "09:30", "10:00", "10:30", "11:00"]);
  });

  it("carries each slot's real instant alongside its wall-clock label", () => {
    const slots = freeSlots(q());
    expect(slots[0]).toEqual({ time: "09:00", startsAt: at("09:00") });
  });

  it("merges several windows on one day", () => {
    const schedule = [mon("09:00", "10:00"), mon("14:00", "15:30")];
    expect(times(freeSlots(q({ schedule })))).toEqual(["09:00", "14:00", "14:30"]);
  });
  ...
  it("ignores windows marked unavailable and other weekdays", () => {
    ...
    expect(times(freeSlots(q({ schedule })))).toEqual([]);
  });
  ...
  it("drops starts inside the minimum advance notice", () => {
    ...
    expect(times(freeSlots(q({ now, rules })))).toEqual(["10:00", "10:30", "11:00"]);
  });
  ...
  it("keeps a buffer on both sides of an existing booking", () => {
    ...
    expect(times(freeSlots(q({ busy, rules, schedule: [mon("08:00", "13:00")] }))))
      .toEqual(["08:00", "08:30", "11:30", "12:00"]);
  });

  it("lets back-to-back sessions touch when there is no buffer", () => {
    const busy = [{ start: at("10:00"), end: at("11:00") }];
    expect(times(freeSlots(q({ busy })))).toEqual(["09:00", "11:00"]);
  });

  it("offers nothing once bookingsToday reaches maxBookingsPerDay", () => {
    const busy = [{ start: at("07:00"), end: at("08:00") }];
    expect(times(freeSlots(q({ busy, bookingsToday: 1, rules: { ...NO_RULES, maxBookingsPerDay: 1 } }))))
      .toEqual([]);
  });

  it("the daily cap looks at bookingsToday, not how many entries are in busy", () => {
    // Two busy intervals (e.g. one spilling over from the previous day, plus a real one), but
    // only one of them actually started today — the cap must not trip on busy.length alone.
    const busy = [{ start: at("07:00"), end: at("07:30") }, { start: at("07:45"), end: at("08:00") }];
    expect(times(freeSlots(q({ busy, bookingsToday: 1, rules: { ...NO_RULES, maxBookingsPerDay: 2 } }))))
      .not.toEqual([]);
  });

  it("an override that closes the day wins over the weekly hours", () => {
    expect(times(freeSlots(q({ override: { isAvailable: false, windows: [] } })))).toEqual([]);
  });

  it("an override with custom hours replaces the weekly hours", () => {
    const override = { isAvailable: true, windows: [{ start: "15:00", end: "16:30" }] };
    expect(times(freeSlots(q({ override })))).toEqual(["15:00", "15:30"]);
  });

  it("an override opens a day the weekly schedule keeps closed", () => {
    const override = { isAvailable: true, windows: [{ start: "10:00", end: "11:00" }] };
    expect(times(freeSlots(q({ date: "2026-09-20", override })))).toEqual(["10:00"]);
  });

  it("uses the service duration: a 90-minute session needs 90 free minutes", () => {
    expect(times(freeSlots(q({ durationMinutes: 90 })))).toEqual(["09:00", "09:30", "10:00", "10:30"]);
  });

  it("returns nothing for a non-positive duration", () => {
    expect(times(freeSlots(q({ durationMinutes: 0 })))).toEqual([]);
  });
  ...
  it("skips the hour that does not exist when clocks go forward (2026-03-29)", () => {
    ...
    expect(times(freeSlots(q({ schedule, date: "2026-03-29" })))).toEqual(["01:00", "01:30", "03:00"]);
  });
  ...
  it("measures a session in real time when clocks go back (2026-10-25)", () => {
    ...
    expect(times(freeSlots(q({ schedule, date: "2026-10-25", durationMinutes: 120 }))))
      .toEqual(["01:00", "01:30"]);
  });
});
